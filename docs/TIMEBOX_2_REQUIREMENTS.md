...
speculative code costs the semester. *You aren't gonna need it* is a rule about code, not
about thinking.
...
except in one bridge file, so gameplay can be rewritten without touching UI. It could and should be split into many
more systems (i.e., event Enums).
...
**Review discipline carries over.** The ring, a review within 24 hours, nobody merging their own work.
...
**A build check.** Ideally continuous integration — an automated build on every pull request. Be warned that Unity in CI needs a
...
on the team can explain. If that is happening, your run log will show it.
Reporting it honestly is a better outcome than hiding it.
